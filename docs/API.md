...
비밀번호는 8~128자이며, 회원가입 시 영문(A–Z/a–z), 숫자, ASCII 기호를 각각 하나 이상 포함해야 합니다. 공백이나 한글은 기호 조건을 충족하지 않습니다. 로그인에는 조합 조건을 강제하지 않아 기존 계정도 사용할 수 있습니다. 표시 이름은 1~80자입니다. 이메일은 Trim 후 소문자로 정규화합니다. 프로필에 passwordHash는 반환하지 않습니다.
